...
public class EnlightenmentCenterLogic {
    ...
    static void runEnlightenmentCenter() throws GameActionException {
        ...
        if (rc.isReady()) {         //this line is important! don't update cooldowns w/o spawning anything
            ...
            if (rc.getRoundNum() < 15 && slandCooldown < 0) {      //don't want to spam slands TOO rapidly
            ...
            else if (rng.nextDouble() < Math.pow(rc.getInfluence(), 0.3) / 10.0 || rng.nextDouble() < slandScore / 30.0) {
                //randomized spawn code
                //note we're not going to spawn slanderers in this state
                //System.out.println("Poli spawn prob: " + (0.5 + slandScore / 30.0));
                toSpawn = (rng.nextDouble() < Math.min(0.75, 0.4 + slandScore / 30.0)) ? RobotType.POLITICIAN : RobotType.MUCKRAKER;

                if (toSpawn == RobotType.POLITICIAN) {
                    if (rng.nextDouble() < 0.8 && bombCooldown < 0 && targetECLoc.x != -1 && targetECLoc.y != -1 
                ...
                //don't do muckspam if we're really low on infl
                int numSpawnSpots = 0;
                for (int i = 0; i < 8; i++) {
                    if (rc.onTheMap(rc.getLocation().add(directions[i])) && rc.senseRobotAtLocation(rc.getLocation().add(directions[i])) == null) {
                        numSpawnSpots++;
                    }
                }
                if (numSpawnSpots > 2)    {
                    //if there's space to spawn mucks
                    int numFoundEnemy = 0;
                    for (int i = 0; i < enemyECLoc.length; i++) {
                        if (enemyECLoc[i].x != -1 && enemyECLoc[i].y != -1)
                            numFoundEnemy += 1;
            ...
            if (shouldSpawn) {
                ...
                //right now EC is biased towards clockwise directions, add a bit of randomness here
                int ccw = 1;
                if (rng.nextInt(2) < 1)
                    ccw = -1;                    

                for (int i = 0; i < directions.length; i++) {
                    Direction dir = directions[(ccw * dirHelper[i] + randDirection + 8) % 8];
                    if (rc.canBuildRobot(toSpawn, dir, spawnInfluence)) {
                        rc.buildRobot(toSpawn, dir, spawnInfluence);
                        if ((toSpawn == RobotType.POLITICIAN && spawnType.equals("Bomb")) || toSpawn == RobotType.MUCKRAKER) {      //we can make more than 100 troops but only keep track of this many
                            if (spawnType.equals("Bomb"))
                                bombCooldown = 10;
                        ...
                        else if (toSpawn == RobotType.POLITICIAN && spawnType.equals("Troop")) {
                            RobotInfo rinfo = rc.senseRobotAtLocation(rc.getLocation().add(dir));
                            for (int j = 0; j < defenderIDs.length; j++) {
                                if (defenderIDs[j] == 0) {
                                    defenderIDs[j] = rinfo.getID();
                                    break;
                                }
                            }
                        }
