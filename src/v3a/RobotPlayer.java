...
public strictfp class RobotPlayer {
    ...
    static int opcodeToInt(OPCODE op) //need to be changed when new opcode added
    {
    	switch(op)
    	{
            ...
            case SLAND: return 8;               //slanderers will display this flag, giving allies true sense
    	}
    	return -1;
    }
...
//    static int expectedNewInfluenceIfNoBid = 150;
    ...
    static void runEnlightenmentCenter() throws GameActionException {
        ...
        if (rc.isReady() && shouldSpawn) {                 
            ...
            if (toBuild == RobotType.POLITICIAN) {
                if (unitType.equals("Bomb")) {
                    ...
                    if(rc.canSetFlag(encodeInstruction(OPCODE.BOMB, unitTarget.x, unitTarget.y, 0))) { //op, xcoord, ycoord, of location to seek
                        rc.setFlag(encodeInstruction(OPCODE.BOMB, unitTarget.x, unitTarget.y, 0));
        ...
        for (int i = 0; i < unitIDs.length; i++) {
            ...
            if (rc.canGetFlag(id)) {
                ...
                else if (opc == OPCODE.STALK)           //can stop tracking this unit
                    unitIDs[i] = 0;
            }
            else
                unitIDs[i] = 0;
            ...
            for (RobotInfo rinfo : nearbyEnemy) {
                if (rinfo.getType() == RobotType.MUCKRAKER) {
                    ...
                    if (isTracked == false) {
                        ...
                        rc.setFlag(encodeInstruction(OPCODE.STALK, 0, 0, curTarget % (1 << 20)));
    ...
    static void runStalk() throws GameActionException {
        ...
        RobotInfo[] nearbyAlly = rc.senseNearbyRobots(senseRadius, ally);
            for (RobotInfo rinfo : nearbyAlly) {
                int flag = rc.getFlag(rinfo.getID());
                if (opcode(flag) == OPCODE.STALK && flag % (1 << 20) == curTarget) {
                    otherStalkers = true;
                    break;
                }
            }
        ...
        if (rc.canSenseRobot(curTarget) && rc.getConviction() > 12 && otherStalkers == false) {
            ...
            for (RobotInfo rinfo : nearbyAlly) {
                if (opcode(rc.getFlag(rinfo.getID())) == OPCODE.SLAND || rinfo.getType() == RobotType.ENLIGHTENMENT_CENTER) {
    ...
    static void runSlanderer() throws GameActionException {
        ...
        rc.setFlag(encodeInstruction(OPCODE.SLAND, 0, 0, 0));
